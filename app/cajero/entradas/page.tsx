  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 flex-1 w-full">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Entradas</h1>
          <p className="text-dark-300">
            Vende entradas (covers), administra eventos y envía QR por email al cliente.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-dark-100 border border-dark-200 rounded-xl p-4">
            <p className="text-sm text-dark-300">Ventas hoy</p>
            <p className="text-2xl font-bold text-white">
              L {todayStats.totalSales.toLocaleString('es-HN', { minimumFractionDigits: 2 })}
            </p>
          </div>
          <div className="bg-dark-100 border border-dark-200 rounded-xl p-4">
            <p className="text-sm text-dark-300">Entradas hoy</p>
            <p className="text-2xl font-bold text-white">{todayStats.totalEntries}</p>
          </div>
          <div className="bg-dark-100 border border-dark-200 rounded-xl p-4">
            <p className="text-sm text-dark-300">Transacciones hoy</p>
            <p className="text-2xl font-bold text-white">{todayStats.totalTransactions}</p>
          </div>
        </div>
