...
export default async function CbTicketsEventosPage() {
  ...
  const featuredEventsPayload = featuredEvents.map((event) => ({
    id: event.id,
    name: event.name,
    date: String(event.date),
    coverImage: event.coverImage,
    coverPrice: Number(event.coverPrice),
    paypalPrice: event.paypalPrice != null ? Number(event.paypalPrice) : null,
    maxEntries: event.maxEntries,
    entriesSoldSum: Number(event.entriesSoldSum ?? 0),
  }))
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 sm:pt-10 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-[1.25fr_0.85fr] gap-5 sm:gap-6">
            ...
            >
              ...
              <FeaturedEventsCarousel events={featuredEventsPayload} />
            </div>
          </div>
        </section>
