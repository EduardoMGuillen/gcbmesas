  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: 'linear-gradient(180deg, #050510 0%, #0a0a1a 40%, #0d0d20 100%)' }}>
      ...
      <nav className="relative z-10 border-b border-white/5">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-center gap-6 sm:gap-10">
          {['Inicio', 'Eventos', 'Ubicacion', 'Horarios', 'Contacto'].map((item) => (
            <a
              key={item}
              href={item === 'Eventos' ? '/eventos' : `https://www.lagrancasablanca.com/#${item.toLowerCase()}`}
              className={`text-xs sm:text-sm font-medium transition-colors ${
                item === 'Eventos' ? 'text-white' : 'text-white/60 hover:text-white'
              }`}
            >
              {item}
            </a>
                  ...
                  <div className="aspect-[4/5] relative overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-bold text-lg" style={{ color: '#c9a84c' }}>${price.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
